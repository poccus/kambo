...
//retorna os horarios do medico que estao sendo utilizados.
$horariosU = array();
...
//$rs = $oConexao->query("SELECT TIME_FORMAT(c.start, '%H:%i') as horario FROM consulta c WHERE  DATE(c.start) = '".$hoje."' AND c.situacao != 6 AND c.idmedico = ".$id." AND '".$horarios[$i]."' BETWEEN TIME_FORMAT(c.start, '%H:%i') AND TIME_FORMAT(c.end, '%H:%i')");
$i = 0;
while($dados = $rs->fetch(PDO::FETCH_OBJ)){
  $horariosU[$i] = $dados->horario;
  $i++; 
}
//print_r($horariosU);

//faz a verificacao para retornar os horarios livres
$horariosLivres  = array_diff($horarios, $horariosU);
//print_r($horariosLivres);


echo json_encode(array_values($horariosLivres));
exit;
?>